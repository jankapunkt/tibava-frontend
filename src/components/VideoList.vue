    ...
    <div class="text-color">
        ...
        <div class="cd-padding">
          <div class="div-width-cb mrg-cb">
            <input
              ...
              v-on:change="onVideoSearch"
            />
          </div>
          <div class="div-width-cb mrg-cb">
            <div class="face-range-slider-slidecontainer text-align-left">
              <input
                class="face-range-slider range-slider-theme-red"
                type="range"
                min="1"
                max="100"
                value="5"
                id="videoDurationFilter"
                v-on:change="onChangeDurationFilter"
              /><label>Duration Filter: {{ durationFilterValue }}</label>
            </div>
          </div>
        </div>
      </div>
      <ul class="annotation">
        <li
          ...
          :key="video[video.length - 1]"
          class="annotation"
        >
          <div
            class="container"
            :id="video[video.length - 1]"
            v-on:click="onVideoSelected(video[video.length - 1])"
          >
            <div class="row">
              <div class="col-sm-3">
                <img
                  :id="'video_' + video[video.length - 1]"
                  class="img-video-frame"
                  src="./../assets/ph_video_1.png"
                  alt="starting"
                />
              </div>
...
export default {
  ...
  data() {
    return {
      videos: [
        [
          "96830434-e9cf-47d3-a385-b90489f9358d",
          "f9 trailer",
          "fast and furious",
          "JAG",
  ...
  methods: {
    ...
    onChangeDurationFilter: function () {
      this.durationFilterValue = parseInt(
    ...
    onVideoSelected: function (videoIdx) {
      ...
      var videoNameIdx = 1;
      this.$root.$refs.MediaPlayerView.vidFileSrc =
        this.videos[videoIdx][videoFileNameIdx];
      document.getElementById("media_player").src =
        this.videos[videoIdx][videoFileNameIdx];
      this.$root.$refs.MediaPlayerView.vidName =
        this.videos[videoIdx][videoNameIdx];
      this.$root.$refs.AnalysisViewerView.getVideoMetaData(
        "media/" + this.videos[videoIdx][videoFileNameIdx],
        this.videos[videoIdx][videoFileNameIdx].split(".")[0]
      );
    },
  },
  mounted: function () {
    axios.post(this.dbServerLink + "getAllVideoRecord").then((responce) => {
