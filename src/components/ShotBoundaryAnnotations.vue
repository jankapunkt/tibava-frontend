  ...
  <div>
    ...
    <div class="global-timeline">
      <table border="0px" width="100%" id="globalTimeline">
        <tr height="15px">
          <th class="scene-timeline-a" colspan="100%">
            <canvas id="timelineGlobal" />
          </th>
        </tr>
        <tr height="25px">
          <th
            v-for="item in curVidShotData"
            :key="item[shotStartFrameIdx]"
      ...
      >
        <tr height="10px" class="border-top border-bottom">
          <th
            ...
            class="table-th-annotation border-down"
          >
            Timelines
          </th>
          <td
            :colspan="curVidShotData.length"
            scope="column"
            class="table-th-col-annotation"
          >
            <canvas id="timeline" />
            <div
        ...
        <tr v-for="tmline in shotTimelines" :key="tmline.id" height="40px">
          <th
            scope="row"
            class="border-down table-th-annotation cursor-default"
            :width="timelineHeadWidth"
          >
            <div class="dropup z-20-i">
              ...
              <ul class="timeline-dropdown dropdown-menu z-20-i">
                <li class="z-20-i">
                  <a
                    href="#"
                    class="timeline-dropdown-anchor z-20-i"
                    v-on:click="duplicateTimeline(tmline[0], tmline[1])"
                    >Duplicate<i
                      class="
                        fa fa-files-o
                        cursor-pointer
                ...
                <li class="z-20-i">
                  <a
                    href="#"
                    class="timeline-dropdown-anchor z-20-i"
                    v-on:click="removeTiemline(tmline[0])"
                    >Remove<i
                      class="
                        fa fa-trash-o
                        cursor-pointer
                ...
                <li class="z-20-i">
                  <a
                    href="#"
                    class="timeline-dropdown-anchor z-20-i"
                    v-on:click="openTimelineEditModal(tmline[0])"
                    >Edit<i
                      class="
                        fa fa-edit
                        cursor-pointer
          ...
          <td
            v-if="curVidShotDataList[tmline[0]]"
            :colspan="curVidShotDataList[tmline[0]].length"
          >
            <div class="timeline-container">
              <div
                class="cursor-pointer"
                v-for="item in curVidShotDataList[tmline[0]]"
                :key="item[shotStartFrameIdx]"
                :style="
                  'width: ' + getPercentage(item) + ' !important; height: 40px'
                "
                @click.self="openModal(tmline[1], item[annotationIdColIdx])"
              >
                <button
                  v-if="item[annotationNameColIdx] != ''"
                  class="annotation-text-wrap annotation-tags-btn"
                  v-on:click="
                    deleteAnnotationValue(tmline[1], item[annotationIdColIdx])
                  "
                >
                  {{ item[annotationNameColIdx]
                  }}<i class="fa fa-times pd-fa-icon"></i>
                </button>
              </div>
            </div>
          </td>
...
-->
    ...
    <div id="timelineOptionsModal" class="modal">
      <div class="modal-content">
        <span
          class="timelineOptionsModalClose text-align-right list-item-pointer"
          >&times;</span
        >
        <div class="container">
          <div class="row">
            <div class="col-sm-12">
              <div class="form-group row">
                <div class="col-sm-5">
                  <label for="dd_annotation_category">
                    Update Timeline Category</label
                  >
                </div>
                <div class="col-sm-4">
                  <div
                    class="dropdown"
                    id="dd_annotation_category"
                    name="dd_annotation_category"
                  >
                    <button
                      type="button"
                      class="btn btn-primary dropdown-toggle col-sm-12"
                      id="dd_annotation_category_button_in_timeline"
                      data-toggle="dropdown"
                    >
                      Select Category
                    </button>
                    <div class="dropdown-menu">
                      <input
                        type="hidden"
                        id="dd_annotation_category_value"
                        name=""
                        value=""
                      />
                      <input
                      ...
                      <a
                        class="dropdown-item"
                        v-for="category in category_names"
                        :key="category[0]"
                        :id="category[0]"
                        :value="category[1]"
                        href="#"
                        v-on:click="
                          onChangeAnnotationCategoryInTimeline($event, category)
                        "
                        >{{ category[1] }}</a
                      >
                    </div>
                ...
                <div class="col-sm-5">
                  <div class="dropdown" id="dd_annotation_category">
                    <button
                      type="button"
                      class="btn btn-primary dropdown-toggle col-sm-12"
                      id="dd_annotation_category_button_in_annotation"
                      data-toggle="dropdown"
                    >
                      Select Category
                    </button>
                    <div class="dropdown-menu">
                      ...
                      <a
                        ...
                        href="#"
                        v-on:click="
                          onChangeAnnotationCategoryInAnnotation(
                            $event,
                            category
                          )
                        "
                      ...
                      <div>
                        ...
                        /><i
                          ...
                          title="New Category"
                          v-on:click="saveAnnotationCategory"
                        ></i>
                      </div>
                    </div>
                  </div>
                ...
                <div class="col-sm-5">
                  <div class="dropdown" id="dd_annotation_item">
                    ...
                    <div class="dropdown-menu">
                      <input
                        type="hidden"
                        id="dd_annotation_item_value"
                        name=""
                        value=""
                      />
                      <a
                        class="dropdown-item"
                        v-for="item in item_names"
                        :key="item[0]"
                        :id="item[0]"
                        :value="item[2]"
                        href="#"
                        v-on:click="onChangeAnnotationItem($event, item)"
                        >{{ item[2] }}</a
                      >
                      <div>
                        <input
...
export default {
  ...
  methods: {
    ...
    addNewTimeline: function () {
      var category_name = document.getElementById(
        "dd_annotation_category_button_in_timeline"
      ).innerText;
      var payload = {
        obj: {
          name: category_name,
          video_id: "d626619f-668f-4fd3-8df0-0ffdf86800bf",
        },
      };
      axios.post(this.dbServerLink + "saveTimeline", payload).then((res) => {
        ...
        if (res.data["status"] == 200) {
          ...
          document.getElementById("timelineOptionsModal").style.display =
            "none";
    ...
    removeTiemline: function (timelineId) {
      ...
      axios.post(this.dbServerLink + "deleteTimeline", payload).then((res) => {
        console.log("on: addNewTimeline: ");
        console.log(res);
        if (res.data["status"] == 200) {
          this.$notifikation.success({
    ...
    updateTimeline: function () {
      ...
      var payload = {
        obj: { category_name: category_name, timeline_id: timelineId },
      };
      axios.post(this.dbServerLink + "updateTimeline", payload).then((res) => {
        console.log("on: addNewTimeline: ");
        console.log(res);
        if (res.data["status"] == 200) {
          this.$notifikation.success({
    ...
    deleteAnnotationValue(timelineId, annotationId) {
      ...
      axios
        .post(this.dbServerLink + "updateTimelineSegment", payload)
        .then((res) => {
          console.log("on: in deleteAnnotationValue ");
          console.log(res);
          if (res.data["status"] == 200) {
            this.$notifikation.success({
              message: "AnnotationItem deleted.",
              duration: this.notifikationDuration,
              style: {
                width: this.notifikationWidth,
                top: this.notifikationTop,
            ...
            this.$notifikation.error({
              message: "AnnotationItem not deleted.",
              duration: this.notifikationDuration,
              style: {
                width: this.notifikationWidth,
                top: this.notifikationTop,
    ...
    selectSaveAnnotationItem() {
      ...
      var payload = {
        obj: {
          annotationId: this.currSegmentId,
          annotationItem: this.currAnnotationItemName,
        },
      };
      axios
        .post(this.dbServerLink + "updateTimelineSegment", payload)
        .then((res) => {
          ...
          if (res.data["status"] == 200) {
            ...
            this.getAllTimelines();
            this.getAllAnnotationCategories(this.defaultCategoryValue);
            var CategoryName =
              document.getElementById("category_name_dd").value;
            document.getElementById("category_name_dd").value = "";
    ...
    saveAnnotationCategory() {
      ...
      if (category_name !== "") {
        ...
        axios
          .post(this.dbServerLink + "saveAnnotationCategory", payload)
          .then((res) => {
    ...
    onChangeAnnotationCategoryInTimeline(event, entry) {
      document.getElementById("dd_annotation_category_value").value =
        event.target.id;
    ...
    onChangeAnnotationItem(event, entry) {
      document.getElementById("dd_annotation_item_value").value =
        event.target.id;
    ...
    saveAnnotationItem() {
      ...
      if (categoryId !== "" && item_name !== "") {
        ...
        axios
          .post(this.dbServerLink + "saveAnnotationItem", payload)
          .then((res) => {
    ...
    getAllAnnotationCategories(CategoryName = "") {
      axios
        .post(this.dbServerLink + "getAllAnnotationCategories")
        .then((res) => {
    ...
    getAllAnnotationItems() {
      var payload = {
        obj: {
          category: document.getElementById("dd_annotation_category_value")
            .value,
        },
      };
      axios
        .post(this.dbServerLink + "getAllAnnotationItems", payload)
        .then((res) => {
    ...
    getAllTimelines() {
      ...
      axios
        ...
        .then((res) => {
          if (res.data["status"] == 200) {
            console.log(
              "getAllTimelinesByVideoId shotboundary Successfully :)"
            );
            for (var k = 0; k < res.data.result.length; k++) {
              this.getTimelineShots(
                res.data.result[k][0],
                res.data.result[k],
                res.data.result
              );
            ...
            this.openModal(this.shotTimelines[0][1], "0", "none");
          } else {
            console.log(
              "getAllTimelinesByVideoId in shotboundary Successfull :("
            );
          }
        });
    },
    openModal(CategoryName, segmentId, dummyBehaviour = "") {
    ...
    updateSelectcategoryAnnotation: function (CategoryName) {
      for (var k = 0; k < this.category_names.length; k++) {
        if (this.category_names[k][1] === CategoryName) {
          document.getElementById("dd_annotation_category_value").value =
            this.category_names[k][0];
          document.getElementById(
            "dd_annotation_category_button_in_annotation"
          ).innerText = this.category_names[k][1];
        }
      }
      this.getAllAnnotationItems();
      document.getElementById(
    ...
    getPercentage(item) {
      var percent = 0;
      var totalFrames =
        this.curVidShotData[this.curVidShotData.length - 1][
          this.shotEndFrameIdx
        ];
      percent =
        ((item[this.shotEndFrameIdx] - item[this.shotStartFrameIdx]) /
          totalFrames) *
        100;
      return percent + "%";
    },
    updatePlayerCurrentTime: function (item) {
      if (item) {
        var calculatedTime =
          this.curVidShotData[item[this.shotSeqIdx]][this.shotStartFrameIdx] /
          this.fps;
    ...
    getTimelineShots: function (timelineId, timeline = [], allTimelines = []) {
      var payload = { obj: { timeline_id: timelineId } };
      axios
        .post(this.dbServerLink + "getTimelineSegmentsByTimelineId", payload)
        .then((res) => {
          console.log("on: getAllAnnotationItems: ");
          console.log(res);
          if (res.data["status"] == 200) {
    ...
    getTimelineShotsAll: function (timelines) {
      ...
      axios
        .post(this.dbServerLink + "getAllTimelineSegments", payload)
        .then((res) => {
          console.log("on: getAllAnnotationItems: ");
          console.log(res);
          if (res.data["status"] == 200) {
