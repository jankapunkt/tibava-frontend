...
export default {
  ...
  data() {
    return {
      ...
      faceDetection: [
        {
          bbox_area: 0.005914713721722364,
          bbox_xywh: [212, 374, 79, 69],
          face_id: 0,
          frame_idx: 39,
        },
        {
          bbox_area: 0.020744357258081436,
          bbox_xywh: [661, 365, 121, 158],
          face_id: 1,
          frame_idx: 118,
        },
        {
          bbox_area: 0.01796874962747097,
          bbox_xywh: [662, 337, 115, 144],
          face_id: 2,
          frame_idx: 119,
        },
        {
          bbox_area: 0.02031249925494194,
          bbox_xywh: [647, 289, 120, 156],
          face_id: 3,
          frame_idx: 120,
        },
        {
          bbox_area: 0.01979166641831398,
          bbox_xywh: [636, 238, 114, 160],
          face_id: 4,
          frame_idx: 121,
        },
        {
          bbox_area: 0.010104166343808174,
          bbox_xywh: [389, 334, 96, 97],
          face_id: 5,
          frame_idx: 155,
        },
        {
          bbox_area: 0.007871094159781933,
          bbox_xywh: [198, 236, 62, 117],
          face_id: 6,
          frame_idx: 155,
        },
        {
          bbox_area: 0.008771701715886593,
          bbox_xywh: [403, 307, 86, 94],
          face_id: 7,
          frame_idx: 156,
        },
      ],
  ...
  methods: {
    ...
    updateFaceOccurance: function (event) {
      ...
      var selected = this.getFaceClusterById(
        cluster_id_computed,
        faceClustering
      );
      for (var l = 0; l < vidShotData.length; l++) {
        ...
        document.getElementById(
          "shot_anno_timeline_1_" + vidShotData[l].shot_id
        ).className = "";
      }
      for (var k = 0; k < faceClustering[selected].face_ids.length; k++) {
        if (faceDetection[k]) {
          ...
          //console.log(timeIdx);
          if (
            frameIdx >= vidShotData[curShotNum].start_frame &&
            frameIdx <= vidShotData[curShotNum].end_frame
          ) {
            ...
            if (
              document
                .getElementById(
                  "shot_anno_timeline_1_" + vidShotData[curShotNum].shot_id
                )
                .className.indexOf(this.occuranceColorTheme) < 0
            ) {
              document.getElementById(
                "shot_anno_timeline_1_" + vidShotData[curShotNum].shot_id
              ).className += " " + this.occuranceColorTheme;
            }
          }
    ...
    checkGettingFaceFramesDone: function (jobId) {
      ...
      axios
        .get(this.$root.$refs.AnalysisViewerView.baseUrl1 + "get_thumbnails", {
          params: { job_id: jobId },
        })
        .then((response) => {
          if (response) {
            if (response.data.status == "SUCCESS") {
              ...
              clearInterval(
                this.$root.$refs.AnalysisViewerView.jobsInProcess[jobId][
                  "intervalHandler"
                ]
              );
              this.$root.$refs.AnnotationListView.faceFrames =
              ...
              for (var i = 0; i < response.data.thumbnails.length; i++) {
                ...
                if (
                  ...
                  document.getElementById(
                    ...
                    "data:image/jpg;base64," + response.data.thumbnails[i].img;
                }
              }
              ...
              for (
                var k = 0;
                k < faceClustering[selected].face_ids.length;
                k++
              ) {
                if (faceDetection[k]) {
                  var frameIdx = faceDetection[k].frame_idx;
                  ...
                  if (
                    ...
                    if (
                      document
                        .getElementById(
                          "shot_anno_timeline_1_" +
                            vidShotData[curShotNum].shot_id
                        )
                        .className.indexOf(this.occuranceColorTheme) < 0
                    ) {
                      document.getElementById(
              ...
              var intervalHandler = setInterval(
                this.checkGettingFaceFramesDone(jobId),
                AppConfig.jobRecallTime
              );
              this.$root.$refs.AnalysisViewerView.jobsInProcess[jobId] = {
                type: "get_face_frames",
    ...
    checkGettingFaceClustersDone: function (jobId) {
      ...
      axios
        .get(this.$root.$refs.AnalysisViewerView.baseUrl1 + "cluster_faces", {
          params: { job_id: jobId },
        })
        .then((response) => {
          if (response) {
            if (response.data.status == "SUCCESS") {
              ...
              var colorShift = 50;
              var colorBase = 150;
              var faceClusterOccuranceThrehold =
                this.defaultOccuranceFilterValues;
              ...
              for (var j = 0; j < faceClusters.length; j++) {
                faceClusters[j]["color"] = [
                  Math.floor(Math.random() * colorBase + colorShift),
                  Math.floor(Math.random() * colorBase + colorShift),
                  Math.floor(Math.random() * colorBase + colorShift),
                ];
                if (
                  faceClusters[j]["occurrences"] >= faceClusterOccuranceThrehold
                ) {
                  newFaceClusters.push(faceClusters[j]);
                }
              }
              this.$root.$refs.DataViewer.faceClustering = newFaceClusters;
              this.$root.$refs.AnnotationListView.faceClustering =
                this.$root.$refs.DataViewer.faceClustering;
              console.log(
                " this.$root.$refs.DataViewer.faceClustering",
                this.$root.$refs.DataViewer.faceClustering
              );
              ...
              var intervalHandler = setInterval(
                this.checkGettingFaceClustersDone(jobId),
                AppConfig.jobRecallTime
              );
              this.$root.$refs.AnalysisViewerView.jobsInProcess[jobId] = {
                type: "get_face_clusters",
                intervalHandler: intervalHandler,
              };
            }
          }
    ...
    checkGettingFacesDone: function (jobId) {
      ...
      axios
        .get(this.$root.$refs.AnalysisViewerView.baseUrl1 + "detect_faces", {
          params: { job_id: jobId },
        })
        .then((response) => {
          if (response) {
            if (response.data.status == "SUCCESS") {
              ...
              if (faceClustering) {
                for (var j = 0; j < faceClustering.length; j++) {
                  ...
                  var facesIds = [
                    faceClustering[j].face_ids[0],
                    faceClustering[j].face_ids[1],
                    faceClustering[j].face_ids[2],
                  ];
                  var thresHold = 0;
                  for (var i = 0; i < facesIds.length; i++) {
                    if (faceClustering[j].occurrences > 4) {
                      facesIds = [
                        faceClustering[j].face_ids[0],
                        faceClustering[j].face_ids[
                          parseInt(faceClustering[j].occurrences / 2)
                        ],
                        faceClustering[j].face_ids[
                          faceClustering[j].occurrences - 2
                        ],
                      ];
                    } else {
                      facesIds = [
                    ...
                    if (faceDetection[facesIds[i]]) {
                      if (faceClustering[j].occurrences > thresHold) {
                        thumbnail_face_frames.push({
                          id: faceClustering[j].cluster_id,
                          idx: faceDetection[facesIds[i]].frame_idx,
                          bbox_xywh: faceDetection[facesIds[i]].bbox_xywh,
                        });
                      }
                    }
                  }
                ...
                axios
                  .post(
                    this.$root.$refs.AnalysisViewerView.baseUrl1 +
                      "get_thumbnails",
                    {
                      video_id: this.$root.$refs.AnnotationListView.videoId,
                      path: this.$root.$refs.AnnotationListView.videoPath,
                      frames: thumbnail_face_frames,
                    }
                  )
                  .then((responce) => {
                    if (responce) {
                      ...
                      var intervalHandler = setInterval(
                        this.checkGettingFaceFramesDone(responce.data.job_id),
                        AppConfig.jobRecallTime
                      );
                      this.$root.$refs.AnalysisViewerView.jobsInProcess[
                ...
                var intervalHandler2 = setInterval(
                  this.checkGettingFacesDone(jobId),
                  AppConfig.jobRecallTime
                );
                this.$root.$refs.AnalysisViewerView.jobsInProcess[jobId] = {
                  type: "get_faces",
                  intervalHandler: intervalHandler2,
                };
              }
            } else if (response.data.status == "PENDING") {
              var intervalHandler3 = setInterval(
                this.checkGettingFacesDone(jobId),
                AppConfig.jobRecallTime
              );
              this.$root.$refs.AnalysisViewerView.jobsInProcess[jobId] = {
                type: "get_faces",
                intervalHandler: intervalHandler3,
              };
            }
          }
        });
        ...
        .then((response) => {
          if (response) {
            if (response.data.status == "SUCCESS") {
              ...
              this.$root.$refs.AnalysisViewerView.shotFrames =
                response.data.thumbnails;
              var frmInnerCnt = {};
              for (var i = 0; i < response.data.thumbnails.length; i++) {
                if (response.data.thumbnails[i].id in frmInnerCnt) {
              ...
              var intervalHandler = setInterval(
                this.checkGettingShotFramesDone(jobId),
                AppConfig.jobRecallTime
              );
              this.$root.$refs.AnalysisViewerView.jobsInProcess[jobId] = {
                type: "get_shot_frames",
                intervalHandler: intervalHandler,
              };
            }
          }
    ...
    getFacesData: function () {
      ...
      axios
        .post(this.$root.$refs.AnalysisViewerView.baseUrl1 + "detect_faces", {
          video_id: this.$root.$refs.AnnotationListView.videoId,
          path: this.$root.$refs.AnnotationListView.videoPath,
        })
        .then((responce) => {
          if (responce) {
            ...
            var intervalHandler = setInterval(
              this.checkGettingFacesDone(responce.data.job_id),
              AppConfig.jobRecallTime
            );
            this.$root.$refs.AnalysisViewerView.jobsInProcess[
              responce.data.job_id
            ] = { type: "get_faces", intervalHandler: intervalHandler };
          }
        });
    },
    getFaceClustersData: function () {
      axios
        .post(this.$root.$refs.AnalysisViewerView.baseUrl1 + "cluster_faces", {
          video_id: this.$root.$refs.AnnotationListView.videoId,
          path: this.$root.$refs.AnnotationListView.videoPath,
        })
        .then((responce) => {
          if (responce) {
            console.log("setting job to get frames for shots");
            var intervalHandler = setInterval(
              this.checkGettingFaceClustersDone(responce.data.job_id),
              AppConfig.jobRecallTime
            );
            this.$root.$refs.AnalysisViewerView.jobsInProcess[
              responce.data.job_id
            ] = { type: "get_face_clusters", intervalHandler: intervalHandler };
          }
        });
    },
    updateShotsList: function () {
      ...
      axios
        .post(this.$root.$refs.AnalysisViewerView.baseUrl1 + "get_thumbnails", {
          video_id: video_id,
          path: vidPath,
        ...
        .then((responce) => {
          if (responce) {
            console.log("setting job to get frames for shots");
            var intervalHandler = setInterval(
              this.checkGettingShotFramesDone(responce.data.job_id),
              AppConfig.jobRecallTime
            );
            this.$root.$refs.AnalysisViewerView.jobsInProcess[
              responce.data.job_id
            ] = { type: "get_shot_frames", intervalHandler: intervalHandler };
            this.getFacesData();
            this.getFaceClustersData();
          }
        });
